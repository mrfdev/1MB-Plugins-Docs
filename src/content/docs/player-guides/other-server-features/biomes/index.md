---
title: "BentoBox Biomes Guide"
description: "Let island owners change configured island biomes through a menu or island command."
---

BentoBox Biomes is a BentoBox add-on used by one or more 1MoreBlock island game modes. It extends an island rather than creating a separate server world of its own.

> Commands and features on 1MoreBlock depend on the current server configuration, world, and your permissions. A command listed here may be limited to certain ranks or contexts.

## What it adds

- Biome selection by island or configured range.
- Menu-driven biome choices with configurable costs and permissions.
- Game-mode-specific availability.

## Commands

| Command | What it does |
| --- | --- |
| `/is biomes` | Opens biome choices for the current island mode. |
| `/ob biomes` | Opens biome choices in OneBlock when enabled. |

## Getting started

1. Enter the relevant island game mode first.
2. Use `/is biomes` or `/ob biomes` to open the feature.
3. Check the game-mode help if the command label differs, because BentoBox commands can be customized per mode.

## Player notes

- Availability, prices, cooldowns, limits, and rewards are controlled by 1MoreBlock configuration.
- An add-on may be enabled for one island mode but not another.
- Use the command root for your current island mode, such as `/is`, `/ob`, `/acid`, `/cave`, or `/skygrid`.

## Official resources

- [BentoBox add-on documentation](https://docs.bentobox.world/en/latest/)
- [BentoBox Biomes source](https://github.com/BentoBoxWorld/Biomes)

## Reference Links

- [Staff and technical reference](/staff-reference/other-server-features/biomes/)
- [1MoreBlock feature notes](https://github.com/mrfdev/1MB-Plugins-Docs/tree/main/catalog/other-server-features/biomes/)
- [Official plugin documentation](https://github.com/BentoBoxWorld/Biomes)
