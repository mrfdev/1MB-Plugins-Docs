---
title: "BentoBox Greenhouses Guide"
description: "Build recognized greenhouse structures that support configured biome or crop effects."
---

BentoBox Greenhouses is a BentoBox add-on used by one or more 1MoreBlock island game modes. It extends an island rather than creating a separate server world of its own.

> Commands and features on 1MoreBlock depend on the current server configuration, world, and your permissions. A command listed here may be limited to certain ranks or contexts.

## What it adds

- Structure detection for configured greenhouse types.
- Greenhouse-specific plant or biome behavior.
- Island-safe construction and ownership checks.

## Commands

| Command | What it does |
| --- | --- |
| `/is greenhouse` | Shows greenhouse help or management for SkyBlock when enabled. |
| `/acid greenhouse` | Shows greenhouse help in AcidIsland when enabled. |

## Getting started

1. Enter the relevant island game mode first.
2. Use `/is greenhouse` or `/acid greenhouse` to open the feature.
3. Check the game-mode help if the command label differs, because BentoBox commands can be customized per mode.

## Player notes

- Availability, prices, cooldowns, limits, and rewards are controlled by 1MoreBlock configuration.
- An add-on may be enabled for one island mode but not another.
- Use the command root for your current island mode, such as `/is`, `/ob`, `/acid`, `/cave`, or `/skygrid`.

## Official resources

- [BentoBox add-on documentation](https://docs.bentobox.world/en/latest/)
- [BentoBox Greenhouses source](https://github.com/BentoBoxWorld/Greenhouses)

## Reference Links

- [Staff and technical reference](/staff-reference/other-server-features/greenhouses/)
- [1MoreBlock feature notes](https://github.com/mrfdev/1MB-Plugins-Docs/tree/main/catalog/other-server-features/greenhouses/)
- [Official plugin documentation](https://github.com/BentoBoxWorld/Greenhouses)
