---
title: "BentoBox Limits Guide"
description: "Apply island-wide limits to configured blocks, entities, and other resources."
---

BentoBox Limits is a BentoBox add-on used by one or more 1MoreBlock island game modes. It extends an island rather than creating a separate server world of its own.

> Commands and features on 1MoreBlock depend on the current server configuration, world, and your permissions. A command listed here may be limited to certain ranks or contexts.

## What it adds

- Per-island block placement limits.
- Entity and mob limits where configured.
- Limit feedback that helps players plan builds before reaching a cap.

## Commands

| Command | What it does |
| --- | --- |
| `/is limits` | Shows current limits for the SkyBlock island. |
| `/ob limits` | Shows current limits for the OneBlock island. |

## Getting started

1. Enter the relevant island game mode first.
2. Use `/is limits` or `/ob limits` to open the feature.
3. Check the game-mode help if the command label differs, because BentoBox commands can be customized per mode.

## Player notes

- Availability, prices, cooldowns, limits, and rewards are controlled by 1MoreBlock configuration.
- An add-on may be enabled for one island mode but not another.
- Use the command root for your current island mode, such as `/is`, `/ob`, `/acid`, `/cave`, or `/skygrid`.

## Official resources

- [BentoBox add-on documentation](https://docs.bentobox.world/en/latest/)
- [BentoBox Limits source](https://github.com/BentoBoxWorld/Limits)

## Reference Links

- [Staff and technical reference](/staff-reference/other-server-features/limits/)
- [1MoreBlock feature notes](https://github.com/mrfdev/1MB-Plugins-Docs/tree/main/catalog/other-server-features/limits/)
- [Official plugin documentation](https://github.com/BentoBoxWorld/Limits)
