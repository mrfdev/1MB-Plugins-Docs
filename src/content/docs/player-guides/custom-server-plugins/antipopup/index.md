---
title: "AntiPopup Guide"
description: "Hide Minecraft's unsafe-server login popup for native Paper 26.2 players."
---

## Introduction

AntiPopup hides Minecraft's blue unsafe-server warning popup when a native 26.2
client joins the 1MoreBlock Paper 26.2 server. Build `006` is deliberately
server-specific and performs only that join-time operation.

## How Players Use It

1. Join the server directly with the native 26.2 client.
2. Continue playing normally after the popup-free login.

There is no command, menu, permission, configuration, or per-player toggle.
Suppression is always active while the plugin is loaded.

## Available Feature

- Hides the unsafe-server popup during the supported native 26.2 login.
- Does not intercept player chat or provide any other gameplay feature.

## Requirements and Limits

AntiPopup has no rewards, costs, cooldowns, usage limits, progression, or
placeholders. Build `006` is certified for Paper 26.2 with a native 26.2 client
after its successful real-client popup and ordinary-chat test. Older clients,
protocol translators, and proxy paths are outside its supported scope.

## Important Notes

- If the popup returns, tell staff the exact Paper build, Java version, client
  version, and AntiPopup JAR name.
- A future Paper or Minecraft version requires a new candidate build and a new
  native-client login test before it is considered supported.
- Players do not need to install a companion mod or plugin.

## Technical Documentation

Administrators and developers can use the
[AntiPopup technical overview](https://github.com/mrfdev/AntiPopup/blob/master/README.md).
The canonical public page is
[docs.1moreblock.com/custom-server-plugins/antipopup/](https://docs.1moreblock.com/custom-server-plugins/antipopup/).

## Reference Links

- [Technical overview](https://github.com/mrfdev/1MB-Plugins-Docs/blob/main/project-docs/antipopup/README.md)
- [Technical documentation folder](https://github.com/mrfdev/1MB-Plugins-Docs/tree/main/project-docs/antipopup/docs/)
