---
title: "SkyGrid Staff Reference"
description: "Public-safe commands, permissions, configuration, integrations, and troubleshooting notes for SkyGrid."
---

SkyGrid is loaded as a BentoBox game-mode add-on. On 1MoreBlock, players use `/skygrid` and staff administer the mode through `/adminskygrid`.
...
| --- | --- | --- |
| `/adminskygrid` | `skygrid.admin.*` | Shows the SkyGrid administration surface on 1MoreBlock. |
...
- The command roots are `/skygrid` for players and `/adminskygrid` for staff; permission nodes keep the `skygrid` namespace.
...
- [Player guide](/player-guides/other-server-features/skygrid/)
- [Curated source notes](https://github.com/mrfdev/1MB-Plugins-Docs/tree/main/catalog/other-server-features/skygrid/)
