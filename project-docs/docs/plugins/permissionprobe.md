...
The `groups` probe reports LuckPerms primary group and direct cached parent group nodes. For exact source-level inheritance and context tracing, use LuckPerms directly with `/lp user <player> info` or `/lp user <player> permission info <node>`.
...
`luckperms.show-direct-user-hints` only reports exact and wildcard nodes cached directly on the user. It does not pretend to replace LuckPerms source tracing for inherited groups and contexts.
