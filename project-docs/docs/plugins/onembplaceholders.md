...
`builtin` placeholders return values generated by the plugin, such as the runtime Minecraft/Paper version, the configured Paper build target, Java version, online player count, date, and time.

`%onemb_paper_version%` and `%onemb_mc_version%` report the Minecraft version from the running server. They update automatically after replacing the Paper server jar and restarting the server. `%onemb_paper_target%` reports the Paper compatibility target used when the plugin was built, while `%onemb_server_engine%` reports the full server engine string.
