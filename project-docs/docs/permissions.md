...
Forage grants `onembcmi.forage.use`, `onembcmi.forage.shop`, `onembcmi.forage.stats`, and `onembcmi.forage.camp` to players by default so `/forage` can open the hub, sell curated PDC-marked tools through the CMI-backed Vault economy provider, show personal progress, view read-only `/forage top` boards, claim configured daily/weekly quest rewards, check a nearby Forage camp, use camp composter turn-ins, and make PDC-marked Forage Dust for bounded growth pulses. Admin permissions default to false. Grant `onembcmi.forage.give` for owner/staff tool creation, `onembcmi.forage.inspect` for reviewing player progression and held tool PDC, `onembcmi.forage.reload` for config/data reloads, and `onembcmi.forage.debug` for tool/debug pages. `onembcmi.forage.admin` includes the admin child nodes, but operator status alone does not receive it from plugin.yml defaults. The plugin does not accept player free-text input in v1, blocks repair/enchant/anvil routes for Forage tools, scans only plain vanilla stacks for camp composter turn-ins, consumes only plain vanilla bonemeal for Forage Dust, and uses chunk exhaustion plus optional WorldGuard global-region checks as early anti-grind guards. Quest reward commands are owner-configured and dispatch only after strict allowed-prefix checks.
