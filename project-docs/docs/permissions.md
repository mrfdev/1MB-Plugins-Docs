...
AutoSell grants `onembcmi.autosell.use`, `onembcmi.autosell.toggle`, and `onembcmi.autosell.preview` to players by default so `/autosell` can open the opt-in GUI, turn the feature on/off, change category/filter/world preferences, and preview normal-inventory items before selling. AutoSell only sells selected pure vanilla items from inventory storage slots and never scans hotbar, offhand, armor, open containers, custom items, storage containers, or hard-blacklisted materials by default. Admin permissions default to false. Grant `onembcmi.autosell.reload` for config/data/worth reloads, `onembcmi.autosell.debug` for held-item eligibility diagnostics, `onembcmi.autosell.warnings` for suspicious volume review, and `onembcmi.autosell.export` for Markdown reports. `onembcmi.autosell.admin` includes those children plus blacklist, category move, inspect, readiness check, and AutoSell boost start/stop commands. Cap permissions are separate: `onembcmi.autosell.cap.20000`, `.100000`, `.250000`, `.500000`, and `.unlimited` increase the daily AutoSell money cap. Operator status alone does not receive admin or cap permissions from plugin.yml defaults.
