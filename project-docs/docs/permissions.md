...
Collect grants `onembcmi.collect.use`, `onembcmi.collect.submit`, `onembcmi.collect.stats`, `onembcmi.collect.leaderboard`, and `onembcmi.collect.rewards` to players by default so `/collect` can open the event GUI, submit configured weekly untouched vanilla items, view standings/personal bests/community progress, and claim earned participation, milestone, and top-10 rewards. Modified stacks with names, lore, damage, enchantments, custom model data, PDC, or other item meta are ignored. Admin permissions default to false. Grant `onembcmi.collect.test` for debug event/week/day/date controls, `onembcmi.collect.inspect` for reviewing stored player event data, `onembcmi.collect.reset` for clearing one player's event data, `onembcmi.collect.reload` for reloading `config.yml`, `events.yml`, and `rewards.yml`, and `onembcmi.collect.report` for exporting Discord-friendly markdown event reports after an event. Reward/submission command hooks are controlled by config allowed-prefix lists and are dispatched from direct server console only, never RCON.
