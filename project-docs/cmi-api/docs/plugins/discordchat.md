...
Open `/discordchat` and click **Top**. Choose **Points**, **Streak**, **Messages**, **Words**, or **Emotes** to see the top ten players. Points ranks current spendable balances; Streak ranks current activity streaks. Messages, Words, and Emotes use the same tracked totals as the existing chat command.
...
`/discordchat top points` and the other metric commands remain available in chat. The menu uses the same ordering and player display names. The **Your Stats** head in the bottom-left corner uses your current Minecraft skin, just like the index and other subpages. Click it to show your DiscordChat status in chat.
...
Use `/discordchat prefix` or `/discordchat suffix`, or choose **Chat Cosmetics** from `/discordchat`. Both commands open a menu with prefixes on the left and suffixes on the right. **Discord** appears only under prefixes, and **Chatty** only under suffixes. Hover over your head at the top centre to see your current prefix, Minecraft name and suffix together. Each choice also previews how it would look, including choices you have not unlocked.

Click an unlocked choice to select it; click the selected choice again to turn it off. **Prefix Off** and **Suffix Off** independently clear each side. A green dye means that side is already off. Its lore directs you to the unlocked name tags (prefixes) or signs (suffixes) below to turn one on; clicking the dye while already off repeats that guidance in chat. When a cosmetic is active, clicking its Off dye removes it. Clicking the selected name tag or sign also turns it off. Switching is free and does not spend points or grant permissions. Locked choices explain the matching reward. Hidden or disabled unlock rewards still provide these explanations without appearing in the shop or becoming purchasable. The **Unlock Rewards** button opens the reward list, where purchases still require their usual confirmation.
...
Purchases pause while either tournament is running. Pyro automatic scheduling stays disabled on 1MB; events start through staff commands, CMI schedules, or a confirmed reward purchase. DiscordChat checks whether an event is running, but cannot predict future CMI schedule commands. An unavailable integration or unreadable tournament status also blocks the purchase. A blocked purchase spends no points. If a start cannot be verified after the command runs, staff review the saved transaction before another tournament purchase can proceed.

The menu shows whether each tournament reward is currently available. An unavailable card cannot be purchased.

### Can I request a day of extra Discord EXP?

Public Beta 2 includes **Discord EXP day request** for **75 points**. Confirming it sends mrfloris an in-game CMI mail asking for a day of extra **MEE6 chat EXP on Discord**. Staff activate the day manually; buying this reward sends the request and does not immediately enable a boost. It does not award DiscordChat points or Minecraft XP.

There is a **24-hour server-wide cooldown**, so one player requests the day for the community. The price and cooldown are configurable. A missing or full staff mailbox blocks the purchase without spending points. If delivery is uncertain, staff review the request before another can be bought. The menu shows the reward's current availability.
...
## Tournament Rewards (Public Beta 2)
...
Before charging and immediately before starting, DiscordChat checks both installed providers through `%pyrofishingpro_istournament%` and `%pyrofarming_istournament%`. Neither may already have a tournament running, including an event started manually or through CMI. Keep `AutoStart.Enabled: false` in both Pyro `tournaments.yml` files on 1MB. DiscordChat never enables automatic scheduling or changes these files.

With Pyro automatic scheduling confirmed off, no automatic countdown is requested. This avoids PyroFarming 1.3.2's missing-scheduler exception and PyroFishingPro's meaningless countdown in manual mode. If another installation enables Pyro's own scheduler, the next event must be more than 15 minutes plus a five-second margin away. Only then are `%pyrofishingpro_timetilltournament%` and the corresponding farming placeholder read; unrecognized countdown formats block purchases. The installed English `d`, `h`, `m`, `s` formats are supported. Future CMI scheduled commands are outside this countdown check, so staff must coordinate their timing.

Pyro does not expose a supported scheduling-mode API. The isolated `DiscordPyroTournamentSchedule` adapter therefore reads the already-loaded public tournament configuration, with strict provider class/version checks: PyroFarming 1.3.2 and the PyroFishingPro 4.9.32 JAR whose descriptor reports 4.9.31. It performs no file I/O or writes, and rechecks loaded settings for every preflight and delivery. Unknown versions, missing configuration, or malformed settings block purchases with a scheduling-mode readiness message; review the adapter and rerun native tests before upgrading either provider. It never treats an unreadable status or a thrown countdown exception as evidence that scheduling is disabled.
...
Tournament tests cover both providers, concrete random selection including a single eligible type, missing dependencies, unreadable status, active and imminent scheduled events, cross-provider blocking, prices, global cooldowns, repeated actions, changes after reservation, exact refunds, uncertain command results, failed persistence after a start, restart/reload recovery and staff acknowledgement without replay. Provider-adapter regression tests use the installed Pyro classes to verify disabled automatic scheduling, malformed/missing settings, unknown versions, configuration replacement on reload, and active manual tournaments. Native tests keep automatic scheduling disabled throughout.
...
For tournament acceptance on the local server, keep automatic scheduling off and verify both new cards and purchase reviews with an eligible ordinary player. Buy each when both providers are idle and no CMI start is planned during the event; confirm the advertised random type starts for 15 minutes and Pyro awards its usual participation/placement prizes. Try both cards during either active event: no points or commands should be consumed. Repeat after reload/restart and with a second player to verify the separate server-wide cooldowns. Use the transaction status/recovery commands for a deliberately interrupted disposable test purchase; never blindly replay an uncertain start. Automated tests cover the ledger/failure paths, while real-player scoring and prize delivery require gameplay acceptance.
