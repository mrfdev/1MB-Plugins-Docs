...
### How do I join a DropParty?

Open `/dropparty` for the schedule and event details. When a party is active, use its travel option to reach the arena. The feature may remain closed until staff enable a reviewed event.

### Why can I not pick up loot while standing at a vent?

Collection requires enough recent movement inside the arena, an eligible game mode, and join access. Move between vents; idling over one geyser does not qualify.

### Where does the loot I collect go?

It is saved in your personal DropParty collection. Leave the party world and run `/dropparty claim` to move it into your normal inventory. Items that do not fit remain stored.

### Why is my claim blocked?

Item claims require AutoSell off and a permitted claim location. Follow the message. If it reports an unresolved previous delivery, ask staff to inspect it before another attempt.
...
```text
jar: 1MB-Lib-DropParty-v1.0.3-639-j25-26.2.jar
Java target: 25
