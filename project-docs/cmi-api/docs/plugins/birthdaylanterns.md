...
### How do I set my birthday and claim a lantern?

Use `/birthday set May 28` or `/birthday set 05-28`, then open `/birthday` to review eligible rewards. Claim through the menu or `/birthday claim <reward-id>`.

### Can I correct my birthday or keep it private?

The default allows one correction with `/birthday set`. Ask staff if another correction is needed. `/birthday visibility private` controls birthday visibility; effects and greetings have separate preferences in the menu.

### What is the difference between a collectible and a wish lantern?

A collectible is a keepsake and does not spend wish charges. A wish lantern has finite uses. Normal right-click opens the menu; sneak-right-click activates the lantern’s behavior.

### Why can I not claim an older server anniversary lantern?

You must have joined before that server milestone date, and the milestone must already have happened. Your personal anniversary rewards use your own first-join date. Check the menu for the eligibility reason.
