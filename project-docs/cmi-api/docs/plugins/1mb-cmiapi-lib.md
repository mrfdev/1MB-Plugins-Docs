...
`FeatureInstallPolicy` controls the first-install `enabled` value for every shared feature jar. The current live allowlist starts enabled; all other and unknown feature ids start with `enabled: false`. A disabled feature stops before registering its descriptor, commands, listeners, tasks, placeholders, hooks, or feature-specific stores, and Paper marks that plugin disabled. Existing config values are never overwritten, so the beta server can keep every feature explicitly enabled while a fresh live installation remains fail closed. Activating a startup-disabled feature requires setting `enabled: true` in its central config and restarting the server.
