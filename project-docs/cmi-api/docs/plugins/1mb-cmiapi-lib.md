...
The library also supports compatibility modules hosted by one normal feature jar. TeamMsg uses this for StaffMsg and NotableMsg; ContentGuard uses it for FilterLab and FilterGuard; Placeholders uses it for Provider, Catalog, Probe, and Health. Hosted modules retain their old config, translation, permission, and placeholder namespaces, while Placeholders intentionally routes all module commands through `/_placeholders`. Their listeners and behavior are governed by the parent lifecycle plus an independent module switch.
...
Console or a sender with `onembcmi.<feature-id>.admin` or `onembcmi.global.config.set` can use `/<feature-command> debug enable true|false`. This performs a live, fail-closed transition and atomically persists the setting. Returning to dormant mode invokes feature shutdown, unregisters listeners and services, cancels owned tasks, closes owned GUI sessions, suppresses placeholders, and restores the restricted lifecycle gateway. A failed activation rolls back to `enabled: false`; missing dependencies or a broken startup configuration still disable the Bukkit plugin as a genuine fault.
