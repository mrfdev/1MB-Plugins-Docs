# Halloween Virus administration
...
CoreProtect 25's `coreprotect:entity_origin` byte-array tag records an ordinary mob's original location for audit logging. HV accepts and preserves that exact tag during natural infection and staff test spawns. Other PDC keys, including other CoreProtect keys, remain protected, as do custom names, scoreboard tags, NPC/pet markers, leashes and mounts. No CoreProtect configuration change is needed. If a test spawn fails after creating its candidate, the command identifies the actual rejection, including a bounded list of blocking PDC key names or an outfit warning to check in console. It does not expose stored values. Older builds could incorrectly report “No eligible variant” for these failures even when the requested boss variant was configured.
...
The shared EventHunts host also caches Coconut/Ghost player profiles, including when players are primarily using HV. Its separate periodic save now skips unchanged profiles and queues changed, detached snapshots for a bounded background writer; it no longer serializes or forces those player files to disk inside the 600-tick server-thread callback. Quit saves use the same worker, with retry and reconnect protection. This change leaves HV's ledger, spent sherd IDs, collection and reward transactions, schedule and settings intact. No reset, restamp or config edit is needed. Storage errors still require staff attention; compare a new Spark recording after deployment to verify that the earlier recurring `CoconutProfileRepository.flushAll` stalls have disappeared.
