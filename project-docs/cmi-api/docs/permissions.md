...
CoconutHunt grants `onembcmi.CoconutHunt.use`, `onembcmi.CoconutHunt.progress`, `onembcmi.CoconutHunt.rewards`, `onembcmi.CoconutHunt.claim`, and `onembcmi.CoconutHunt.shop` to players by default so `/coconut` can open from any configured information world, show personal/wave/streak/community/point progress, claim eligible rewards in configured reward worlds, and use the confirmation-gated MobHat shop. Setup, discovery, and proximity visuals remain restricted to the production event world/regions unless isolated debug mode explicitly allows any world. `onembcmi.CoconutHunt.admin` and all staff children default false, including for operators. Grant `onembcmi.CoconutHunt.admin.reload` for full file/runtime reload, `.admin.debug` for isolated debug/date/day controls, `.admin.event` for validation/lifecycle/immutable snapshot actions, `.admin.coconut` for setup heads and registry/wave management, `.admin.inspect` for player review and recorded-purchase regrant, `.admin.reset` for mandatory dry-run/confirm player resets, and `.admin.report` for event exports. Child permissions work independently without the parent; direct server console remains trusted for non-player admin actions. CoconutHunt validates every configured command list against narrow allowed prefixes before persisting a claim or purchase, and no command is supplied by a player.

AFKShrine grants `onembcmi.afkshrine.use`, `onembcmi.afkshrine.claim`, `onembcmi.afkshrine.rewards`, `onembcmi.afkshrine.trade`, `onembcmi.afkshrine.tools`, `onembcmi.afkshrine.top`, and `onembcmi.afkshrine.style` to players by default so the public feature can show status, claim pending AFKShrine tokens, view/trade configured rewards, open the captured-token tools preview, use leaderboards, view seasonal/community progress, open the collection album, and select only unlocked presets. `/afkshrine rewards` and `/afkshrine trade <reward>` show read-only reward readiness details before `confirm` spends claimed tokens, including claimed trade balance, pending tokens, missing tokens, one-time/repeatable state, already-claimed one-time rewards, disabled command rows, and command count. `/afkshrine trade` without a reward id opens the rewards overview. `/afkshrine tools claim <action>` follows the same preview-first pattern before `confirm` consumes exact captured AFKShrine items such as the AFK Shrine Token. `/afkshrine milestones` uses the existing `use` permission and teaches players which time, biome, safety, risk, adventure, weather, seasonal, dimension pilgrim, collection album, normal streak, and Rested Return claim-streak goals exist, how they are earned, and which repeat rule is active. `/afkshrine album` uses that same permission and shows read-only collected/open milestone entries with repeat counts, reset windows, and saved-history entries; `/afkshrine resets` uses it too and explains which daily, weekly, monthly, lifetime, ready, used, inactive, or complete goals can create pending tokens. Weather quest sets, seasonal quest sets, always-available shrine quests, weekly biome circuits, monthly biome wanderer goals, collection-set rewards, claim-based Rested Return streaks, community milestone celebrations, rested bonus state, no-repeat zone history, danger-score candidate calculations, and tiny bed-rest rewards are config-backed and use the existing player flow; they do not add extra player permissions. Rare preset unlocks remain staff-configured through optional `hooks.progress.specific` LuckPerms commands and exact style nodes such as `onembcmi.afkshrine.style.lantern`, `.aurora`, `.prism`, or `.echo`. Bed rest still requires `onembcmi.afkshrine.use`, uses very low caps by default, and skips the normal full AFK reward path while the player is sleeping. AFK progress recovery is config-backed and uses playerdata markers rather than extra permissions. `onembcmi.afkshrine.preview` defaults to false and gates `/afkshrine preview`, preview tab completion, preview help entries, and preview links in `/afkshrine presets`. Non-default preset nodes such as `onembcmi.afkshrine.style.mint`, `.aurora`, `.prism`, `.sunrise`, and `.storm` also default to false; grant exact style nodes or `onembcmi.afkshrine.style.*` when staff intentionally unlocks visual tiers. Staff live-readiness and passive exports use `onembcmi.afkshrine.admin.check`, `onembcmi.afkshrine.admin.report`, and `onembcmi.afkshrine.admin.journal`; monthly journals are read-only exports built from audit logs for balancing/support review. Captured item setup uses `onembcmi.afkshrine.admin.capture`; repeated-zone review uses `onembcmi.afkshrine.admin.suspicious`; per-player persistent state review uses `onembcmi.afkshrine.admin.audit`.
