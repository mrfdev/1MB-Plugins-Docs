...
AFKShrine grants `onembcmi.afkshrine.use`, `onembcmi.afkshrine.claim`, `onembcmi.afkshrine.rewards`, `onembcmi.afkshrine.trade`, `onembcmi.afkshrine.tools`, `onembcmi.afkshrine.top`, and `onembcmi.afkshrine.style` to players by Bukkit default so the public feature can show status, claim pending AFKShrine tokens, view/trade configured rewards, open the captured-token tools preview, use leaderboards, view seasonal/community progress, open the collection album, and select only unlocked presets. Servers that keep upgrades in beta should explicitly deny `onembcmi.afkshrine.tools` to their normal group. `onembcmi.afkshrine.books` defaults to false and independently grants only the lore-book category, `/afkshrine books`, lore-book previews, and final lore-book confirmations; it does not expose spear, shield, or other tool actions. Holders of `.tools` continue to inherit functional access to lore books without needing the extra node. `/afkshrine rewards` and `/afkshrine trade <reward>` show read-only reward readiness details before `confirm` spends claimed tokens, including claimed trade balance, pending tokens, missing tokens, one-time/repeatable state, already-claimed one-time rewards, disabled command rows, and command count. `/afkshrine trade` without a reward id opens the rewards overview. `/afkshrine tools claim <action>` and `/afkshrine books claim <action>` follow the same preview-first pattern before `confirm` consumes exact captured AFKShrine items such as the AFK Shrine Token. `/afkshrine milestones` uses the existing `use` permission and teaches players which time, biome, safety, risk, adventure, weather, seasonal, dimension pilgrim, collection album, normal streak, and Rested Return claim-streak goals exist, how they are earned, and which repeat rule is active. `/afkshrine album` uses that same permission and shows read-only collected/open milestone entries with repeat counts, reset windows, and saved-history entries; `/afkshrine resets` uses it too and explains which daily, weekly, monthly, lifetime, ready, used, inactive, or complete goals can create pending tokens. Weather quest sets, seasonal quest sets, always-available shrine quests, weekly biome circuits, monthly biome wanderer goals, collection-set rewards, claim-based Rested Return streaks, community milestone celebrations, rested bonus state, no-repeat zone history, danger-score candidate calculations, and tiny bed-rest rewards are config-backed and use the existing player flow. Rare preset unlocks remain staff-configured through optional `hooks.progress.specific` LuckPerms commands and exact style nodes such as `onembcmi.afkshrine.style.lantern`, `.aurora`, `.prism`, or `.echo`. Bed rest still requires `onembcmi.afkshrine.use`, uses very low caps by default, and skips the normal full AFK reward path while the player is sleeping. AFK progress recovery is config-backed and uses playerdata markers rather than extra permissions. `onembcmi.afkshrine.preview` defaults to false and gates `/afkshrine preview`, preview tab completion, preview help entries, and preview links in `/afkshrine presets`. Non-default preset nodes such as `onembcmi.afkshrine.style.mint`, `.aurora`, `.prism`, `.sunrise`, and `.storm` also default to false; grant exact style nodes or `onembcmi.afkshrine.style.*` when staff intentionally unlocks visual tiers. Staff live-readiness and passive exports use `onembcmi.afkshrine.admin.check`, `onembcmi.afkshrine.admin.report`, and `onembcmi.afkshrine.admin.journal`; monthly journals are read-only exports built from audit logs for balancing/support review. Captured item setup uses `onembcmi.afkshrine.admin.capture`; repeated-zone review uses `onembcmi.afkshrine.admin.suspicious`; per-player persistent state review uses `onembcmi.afkshrine.admin.audit`. Bounded live-test grants use `onembcmi.afkshrine.admin.debug`; they are audited separately and never count as organic AFK progress.
...
AutoSell grants `onembcmi.autosell.use`, `onembcmi.autosell.toggle`, and `onembcmi.autosell.preview` to players by default so `/autosell` can open the opt-in GUI, turn the feature on/off, change category/filter/world/trigger preferences, preview normal-inventory items before selling, use the quest hub for repeatable daily, weekly, and monthly AutoSell quests and claimable rewards, view the `/autosell milestones` tree for one-time lifetime and bulk-batch goals, and open `/autosell caps` to spend broker points on permanent daily cap unlocks. Background selling requires both `onembcmi.autosell.use` and `onembcmi.autosell.toggle`; both are checked when a batch is scheduled and again before inventory mutation. Removing either permission turns an enabled profile off and saves that state, so restoring access requires a fresh player opt-in. AutoSell only sells selected pure vanilla items from inventory storage slots and never scans hotbar, offhand, armor, open containers, custom items, storage containers, or hard-blacklisted materials by default. Milestone rewards are config-driven and claimed once per player; quest rewards are period-based, reset by daily, weekly, or monthly quest period, and can either pay automatically or wait for manual claiming depending on config. Both reward systems may only run configured direct-console commands whose prefixes are allowlisted. Admin permissions default to false. Grant `onembcmi.autosell.reload` for config/data/worth reloads, `onembcmi.autosell.debug` for held-item eligibility diagnostics, `onembcmi.autosell.warnings` for suspicious volume review, and `onembcmi.autosell.export` for Markdown reports. `onembcmi.autosell.admin` includes those children plus blacklist, category move, inspect, readiness check, compact economy report, chunk heatmap review, passive tuning observation/report commands, scheduled Happy Hour status, and AutoSell boost start/stop commands. Cap permissions are separate: `onembcmi.autosell.cap.20000`, `.100000`, `.250000`, `.500000`, and `.unlimited` increase the daily AutoSell money cap. Purchased cap unlocks are stored in AutoSell player data and can exceed the player's current group cap unless a permission grants more. Operator status alone does not receive admin or cap permissions from plugin.yml defaults.
...
DropParty grants `onembcmi.dropparty.use`, `onembcmi.dropparty.join`, `onembcmi.dropparty.claim`, and `onembcmi.dropparty.stats` to players by default, but the feature itself installs dormant with `enabled: false`. These nodes open the schedule GUI, permit active-arena travel and moving collection, deliver stored vault items outside the event world, and expose personal/top statistics once staff activate the feature. All `onembcmi.dropparty.admin*` nodes default false, including the `menu`, `edit`, `start`, `geyser`, `loot`, `mini`, `recovery`, `reload`, and `debug` children. The parent grants all children. Recovery permission is especially sensitive: staff must inspect an unresolved claim before choosing `refund` for a delivery that never happened or `finalize` for one that did. Operator status alone does not grant DropParty administration.
